...
export function PublicQuotePage({ token }: { token: string }) {
  ...
  const isOwner = Boolean(user && quote.data?.ownerId === user.uid);
  const canView = isOwner || verified;
  ...
  async function downloadPdf() {
    ...
    await generateDocumentPdf({
      ...
      logoDataUrl: quote.data.logoDataUrl
    });
  }
          ...
          {canView ? (
            ...
            <div className="flex min-h-[360px] flex-col items-center justify-center text-center">
              ...
              <p className="mt-2 max-w-md text-sm leading-6 text-muted">
                Enter your email to verify this quote.
              </p>
              <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/45 p-4">
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                <h2 className="text-lg font-semibold text-ink">Verify email</h2>
                <p className="mt-1 text-sm text-muted">Enter your email to verify this quote.</p>
              </div>
              <button
            ...
            {message ? <p className="mt-3 text-sm text-danger-600">{message}</p> : null}
          </form>
        </div>
      ) : null}
    </main>
      ...
      <div className="flex flex-col gap-6 border-b border-line pb-6 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <img
            ...
            src={quote.logoDataUrl || "https://www.shreecleaning.com/logo.png"}
          />
