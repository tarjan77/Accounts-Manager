...
import { useCustomers, useItems, useQuotes } from "@/lib/hooks";
import type {
  CatalogItem,
  Customer,
  InvoiceLineItem,
...
function blankQuote(existingCount: number): QuoteInput {
  return {
    publicToken: newToken(),
    quoteNumber: nextDocumentNumber("QT", existingCount),
    customerId: "",
    customerName: "",
    customerEmail: "",
    customerPhone: "",
...
export function QuotesPage() {
  ...
  const [form, setForm] = useState<QuoteInput>(() => blankQuote(0));
  ...
  function openNew() {
    setEditing(null);
    setForm(blankQuote(quotes.data.length));
    setMessage("");
    setFormOpen(true);
  }
  ...
  return (
    <>
      ...
      <section className="mb-5 grid gap-3 md:grid-cols-3">
        <MiniStat label="Sent quotes" value={totals.sent.toString()} />
        <MiniStat label="Accepted quotes" value={totals.accepted.toString()} />
        ...
        <div className="flex items-center justify-between gap-3 border-b border-line p-5">
          <div>
            ...
            <p className="mt-1 text-sm text-muted">
              Save drafts, send customer review links, and watch accepted or declined status update here.
            </p>
          </div>
        </div>
  ...
  return (
    <form className="space-y-5" onSubmit={handleSubmit}>
      ...
      <section className="grid gap-4 lg:grid-cols-4">
        ...
        <label className="block">
          <span className="field-label">Job time optional</span>
          <input
            className="field"
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_320px]">
        ...
        <div className="rounded-lg border border-line bg-mist p-5">
          ...
          <p className="mt-3 text-xs leading-5 text-muted">
            Sending opens a ready email draft with the quote review link.
          </p>
        </div>
      </div>
    </form>
