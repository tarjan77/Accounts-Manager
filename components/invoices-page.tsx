...
function blankInvoice(existingInvoices: BusinessInvoice[]): BusinessInvoiceInput {
  return {
...
export function InvoicesPage() {
  ...
  const [form, setForm] = useState<BusinessInvoiceInput>(() => blankInvoice([]));
  ...
  function openNew() {
    setEditing(null);
    setForm(blankInvoice(invoices.data));
    setMessage("");
    setFormOpen(true);
  }
  ...
  return (
    <>
      ...
      <section className="mb-5 grid grid-cols-4 gap-2 sm:gap-3">
        <MiniStat label="Outstanding" value={formatCurrency(summary.outstanding)} />
        ...
        <MiniStat label="Paid" value={formatCurrency(summary.paid)} />
      </section>
...
function MiniStat({
  ...
  return (
    <div className="rounded-lg border border-line bg-white p-2.5 sm:p-4">
      ...
      <p className={`mt-1 truncate text-base font-semibold sm:mt-2 sm:text-2xl ${tone === "amber" ? "text-amber-500" : "text-ink"}`}>
        {value}
      </p>
    </div>
  );
